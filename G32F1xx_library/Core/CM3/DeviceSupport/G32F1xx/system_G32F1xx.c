...
#define SYSPLLCLKSEL_Val      1//0x00000001	//external OSC
#define SYSPLL_SETUP          1
#define SYSPLLCTRL_Val        0x00004049//9±∂∆µ
#define MAINCLKSEL_Val        0x00000002	//Main clock source is PLL out
...
void SystemCoreClockUpdate (void)            /* Get Core Clock Frequency      */
{
  ...
  if(((GT_SYSCON->RCCCFGR>>8)&0x7)==4)
	  APB1Clock = SystemCoreClock/2;
  else if(((GT_SYSCON->RCCCFGR>>8)&0x7)==5)
	  APB1Clock = SystemCoreClock/4;
  else if(((GT_SYSCON->RCCCFGR>>8)&0x7)==6)
	  APB1Clock = SystemCoreClock/8;
  else if(((GT_SYSCON->RCCCFGR>>8)&0x7)==7)
	  APB1Clock = SystemCoreClock/16;
  else
	  APB1Clock = SystemCoreClock;
  
  if(((GT_SYSCON->RCCCFGR>>11)&0x7)==4)
	  APB2Clock = SystemCoreClock/2;
  else if(((GT_SYSCON->RCCCFGR>>11)&0x7)==5)
	  APB2Clock = SystemCoreClock/4;
  else if(((GT_SYSCON->RCCCFGR>>11)&0x7)==6)
	  APB2Clock = SystemCoreClock/8;
  else if(((GT_SYSCON->RCCCFGR>>11)&0x7)==7)
...
// #endif
  ...
  GT_WWDT->MOD  = 0x00;  
  GT_WWDT->FEED = 0xAA;		/* Feeding sequence */
  GT_WWDT->FEED = 0x55;
  GT_IWDT->MOD  = 0x00;  
  GT_IWDT->FEED = 0xAA;		/* Feeding sequence */
  GT_IWDT->FEED = 0x55;
  ...
  GT_SYSCON->PDRUNCFG     &= ~(1 << 8);          /* Power-up System Osc      */
  GT_SYSCON->SYSOSCCTRL    = SYSOSCCTRL_Val;//Õ‚≤øæß’Ò ‰»Î2.5-16MHz
...
//  while (!(GT_SYSCON->SYSPLLSTAT & 0x01));	   /* Wait Until PLL Locked    */
  ...
  GT_FMC->RDCYCCFG				 = 0x3;
  GT_SYSCON->MAINCLKUEN    = 0x01;               /* Enable MCLK Source change*/
  GT_SYSCON->MAINCLKSEL    = MAINCLKSEL_Val;     /* Select PLL Clock Output  */
  GT_SYSCON->MAINCLKUEN    = 0x00;               /*Disable MCLK Source change*/

  GT_SYSCON->SYSAHBCLKDIV  = SYSAHBCLKDIV_Val;
  GT_SYSCON->UARTCLKDIV    = UARTCLKDIV_Val;
//  while (!(GT_SYSCON->MAINCLKUEN & 0x01));     /* Wait Until Updated       */
  ...
  GT_SYSCON->SYSAPB2CLKCTRL = 0x1ff;				//Enable AFIO & GPIOA - GPIOG clock
	
	//Release PA8/PA9, these two IO pull-down by DAC1/DAC2.
	#if 1
	GT_SYSCON->SYSAPB1CLKCTRL |= 1<<29;       //DAC CLK on.
	GT_SYSCON->PDRUNCFG       &= ~(1<<4);     //DAC analog power on.
	GT_DAC->DAC1CTRL = 0x1048;
	GT_DAC->DAC1BUF  = 0x1ff;
	GT_DAC->DAC2CTRL = 0x1048;
	GT_DAC->DAC2BUF  = 0x1ff;	
	#endif
  //GT_SYSCON->SYSAPB1CLKCTRL = 0xFFFFFFFF;
	
	
	//Stop when PB2/Boot1 pin is 0
	//GT_SYSCON->SYSAHBCLKCTRL |= 0x3<<2;	//Enable GPIOA/GPIOB
	//if ((GT_GPIOB->PIN&0x00000004)==0x00000000)
	//	while(1);
}
